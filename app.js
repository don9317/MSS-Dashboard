...
function parseReservationDate(raw) {
  ...
  // Fallback (less reliable). Prefer the two formats above.
  const d = new Date(s);
  if (!isNaN(d)) return d;
  return null;
}
...
function parseStartDateTime(resDate, timeStr) {
  ...
  if (ampmMatch) {
    ...
    const ampm = ampmmatchSafe(ampmMatch[3]);
...
function parseCSV(text) {
  ...
  if (missing.length > 0) {
    throw new Error("Missing expected columns in CSV: " + missing.join(", "));
  }
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const isFailed = String(paidStatus || "").trim().toLowerCase() === "failed";

    const resDateStr = resDate ? ymdFromLocalDate(resDate) : "";

    rows.push({
      name: name || "",
      area: area || "",
      reservationDate: resDate, // LOCAL date object at midnight
      reservationDateStr: resDateStr, // YYYY-MM-DD (local-safe)
...
// ---------- Storage ----------
// Store reservationDate as YYYY-MM-DD (no timezone). This prevents date shifting.

function saveReservationsToStorage() {
  const simple = reservations.map((r) => ({
    ...r,
    // Store DATE ONLY (YYYY-MM-DD)
    reservationDate: r.reservationDateStr || (r.reservationDate ? ymdFromLocalDate(r.reservationDate) : null),
    // Store startDateTime as ISO (fine to keep time)
...
function loadReservationsFromStorage() {
  ...
  try {
    ...
    reservations = arr.map((r) => {
      const resDate = r.reservationDate ? parseReservationDate(r.reservationDate) : null;
      const startDateTime = r.startDateTime ? new Date(r.startDateTime) : null;

      return {
        ...r,
        reservationDate: resDate,
        reservationDateStr: resDate ? ymdFromLocalDate(resDate) : (r.reservationDate ? String(r.reservationDate).slice(0, 10) : ""),
        startDateTime,
        startHour: startDateTime ? startDateTime.getHours() : (r.startHour ?? null),
      };
    });
  } catch (e) {
...
function getDateRange(option) {
  ...
  if (option === "today") return { start: ref, end: ref };
  if (option === "yesterday") {
...
function buildDashboard(rangeKey) {
  ...
  statusEl.textContent =
    ...
    (ref ? ymdFromLocalDate(ref) : "n/a") +
    ". Current filter: " +
    rangeKey.replace(/_/g, " ");

  document.getElementById("hourly-label").textContent =
    ymdFromLocalDate(start) + " to " + ymdFromLocalDate(end);
  ...
  rangeRows.forEach((r) => {
    ...
    if (!hourlyMap[h]) hourlyMap[h] = { rentals: 0, revenue: 0 };
  ...
  if (!hourlyEntries.length) {
    ...
    hourlyEntries.forEach(([hourStr, val]) => {
      ...
      const label =
        hourNum >= 0 ? `${hourNum.toString().padStart(2, "0")}:00` : "Unknown";
      const tr = document.createElement("tr");
      tr.innerHTML = `
        <td>${label}</td>
  ...
  rangeRows.forEach((r) => {
    ...
    if (!payMap[key]) payMap[key] = { rentals: 0, revenue: 0 };
  ...
  const periodMetrics = [
    { metric: "Rentals", current: currentRentals, prior: priorRentals, isMoney: false },
    { metric: "Revenue", current: currentRevenue, prior: priorRevenue, isMoney: true },
  ];
  periodMetrics.forEach((row) => {
    const delta = row.current - row.prior;
    const isPos = delta >= 0;
    const badgeClass = isPos ? "badge green" : "badge red";
    const label = row.isMoney ? formatCurrency(Math.abs(delta)) : Math.abs(delta);
    const sign = isPos ? "+" : "-";
    const tr = document.createElement("tr");
    tr.innerHTML = `
      <td>${row.metric}</td>
      <td class="text-right">${row.isMoney ? formatCurrency(row.current) : row.current}</td>
      <td class="text-right">${row.isMoney ? formatCurrency(row.prior) : row.prior}</td>
      <td class="text-right"><span class="${badgeClass}">${sign}${label}</span></td>
    `;
  ...
  let lastYtdEnd;
  try {
    lastYtdEnd = new Date(lastYear, endDate.getMonth(), endDate.getDate());
  } catch (e) {
  ...
  ytdBody.innerHTML = "";
  const ytdMetrics = [
    { metric: "Rentals", current: ytdRentals, last: lastYtdRentals, isMoney: false },
    { metric: "Revenue", current: ytdRevenue, last: lastYtdRevenue, isMoney: true },
  ];
  ytdMetrics.forEach((row) => {
    ...
    tr.innerHTML = `
      <td>${row.metric}</td>
      <td class="text-right">${row.isMoney ? formatCurrency(row.current) : row.current}</td>
      <td class="text-right">${row.isMoney ? formatCurrency(row.last) : row.last}</td>
      <td class="text-right"><span class="${badgeClass}">${sign}${label}</span></td>
    `;
    ytdBody.appendChild(tr);
  });

  // Q3/Q4 below unchanged from your original
  // (Keeping your existing logic for failed payments, sport mix, and setup.)
  ...
  document.getElementById("failed-outstanding-caption").innerHTML =
    '<span class="arrow">▲</span> ' + failedAll.length + " open items";

  // Q4: by sport / setup
  const sportMap = {};
  rangeRows.forEach((r) => {
    const key = (r.sport || "Unknown").trim() || "Unknown";
    if (!sportMap[key]) sportMap[key] = { rentals: 0, revenue: 0 };
  ...
  rangeRows.forEach((r) => {
    ...
    const key = (r.sport || "Unknown").trim() || "Unknown";
    if (!setupMap[key]) setupMap[key] = { rentals: 0, revenue: 0 };
  ...
  let setupCount = 0;
  setupEntries.forEach((e) => (setupCount += e[1].rentals));
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // IMPORTANT: parse custom inputs as LOCAL dates (not UTC).
  document.getElementById("custom-start").addEventListener("change", (e) => {
    if (!e.target.value) return;
    customRange.start = parseReservationDate(e.target.value);
    if (customRange.end) buildDashboard("custom");
  });

  document.getElementById("custom-end").addEventListener("change", (e) => {
    if (!e.target.value) return;
    customRange.end = parseReservationDate(e.target.value);
    if (customRange.start) buildDashboard("custom");
  });
